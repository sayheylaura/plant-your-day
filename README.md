...
I'm using this project to learn FastAPI (and Python) and Vue. It's a work in progress, so expect things to change and break.
...
```bash
git clone git@github.com:sayheylaura/plant-your-day.git && cd plant-your-day
```

This app is containerized with Docker and uses a dev container. It uses environment variables to configure the database connection.

If you're opening the project in a dev container in your editor for the first time, a `.env` file is created automatically from `.env.example`. Edit it with your database credentials, then Rebuild and Reopen the dev container (or run "Reopen in Container" again) so the app can read them. After that, start the backend and frontend manually as described below.
...
After this, all dependencies are installed automatically when you open the project in your editor. However, you need to start the servers manually, so you can see the logs and errors in real time.

### 🐍 Setting up the backend

To get the FastAPI server up and running, run these commands in your editor's integrated terminal:
...
To get the Vue.js frontend up and running, run these commands in your editor's integrated terminal:
